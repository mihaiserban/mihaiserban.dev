---
slug: governance-worker-synthesis-assembly-reconstruction
title: "Orchestrator Agents Are a Local Maximum: What 11 arXiv Papers Say to Build Instead"
date: "2026-07-15T20:45+03:00"
hidden: false
tags: ["ai-agents", "llm", "multi-agent", "context-window", "research"]
---

My coding agents used a pattern I suspect many of you recognize: an orchestrator agent plans the work, spawns minion sub-agents, collects their results, and synthesizes the final answer. It works fine for short tasks. But on anything long-horizon — a multi-file refactor, a cross-repo analysis, a research sprint — it falls apart.
...
**[ClawArena-Team](https://arxiv.org/abs/2606.31174)** (Jun 2026) built a benchmark to measure a single LLM's ability to manage sub-agents. Their finding: **no model exceeds 50% workspace-permission precision.** The bottleneck isn't model capability — it's the architecture of delegating through a single constrained context window. Cost and management quality were essentially decoupled: API spend varied over 100× while management scores clustered in a tight 9.9-point band.

**[ExtAgents](https://arxiv.org/abs/2505.21471)** (ACL 2026) showed that distributing knowledge input across multiple agents in parallel — rather than funneling everything through one orchestrator — significantly outperforms single-context approaches, regardless of whether the input fits in the context window.
...
**[The Illusion of Agentic Complexity](https://arxiv.org/abs/2606.30524)** (ICSME 2026) delivered a sobering finding: for README generation, a single-agent pipeline matched multi-agent quality while **reducing token consumption by 86% and running at 2× speed.** Multi-agent only won on structural consistency. More agents is not uniformly better.
...
**Governance** never does per-unit work and never synthesizes results. It only plans and delegates. Each worker brief is minimal: a task spec path and a result path. Workers write structured results to YAML files and return only a short confirmation — not the full result.
...
The before-and-after on a typical multi-file refactor:
